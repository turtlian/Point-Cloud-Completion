...
class decoder_mlp(nn.Module):
    ...
    def forward(self, feature):
        ...
        x = F.tanh(x)
        x = x.view(-1, self.decoder_feature_dim, tree_arch[0])
...
class decoder_mlp_conv(nn.Module):
    ...
    def forward(self, x, feature):
        for i, num_node in enumerate(tree_arch[1:]):
            ...
            x = F.tanh(x)
            total_node = np.prod([int(k) for k in tree_arch[:i+2]])
            if i != len(tree_arch[1:])-1:
